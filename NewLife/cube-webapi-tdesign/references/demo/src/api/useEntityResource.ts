...
export interface DataField {
  ...
  /** 链接/操作列模板 URL（如 Link/Token/Log：`url="/Admin/UserConnect?userId={ID}"`），itemType=link 或 url 非空时渲染为行级链接，{ID}/{Id} 替换为行主键 */
  url?: string | null;
  /** 列水平对齐：GetPage 约定 0=左 1=中 2=右（映射到 t-table col.align 的 left/center/right） */
  textAlign?: number | null;
  /** 列表列最大宽度（px）；0/空时兜底用 length（列表场景 length 语义为列宽上限），不限宽则自适应 */
  maxWidth?: number | null;
  /** 链接/操作列 tooltip 标题（itemType=link 时取此作为链接文字的 title） */
  title?: string | null;
  /** 列头 tooltip 标题（鼠标悬停列头展示） */
